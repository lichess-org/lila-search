...
class ESGameIngestor(elastic: ESClient[IO], botCache: BotUserCache)(using LoggerFactory[IO])
  ...
  private val humanFormat = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm").withZone(ZoneOffset.UTC)
  private def formatInstant(t: Instant): String =
    s"${humanFormat.format(t)} (${t.getEpochSecond})"
    ...
    stream
      .evalMap: result =>
        botCache.get.flatMap: botIds =>
          ...
          val lastCreatedAt = result.toIndex.lastOption
            .map(_.createdAt)
            .fold(""): t =>
              s", last createdAt: ${formatInstant(t)}"
          Logger[IO].info(s"Indexing ${result.toIndex.size} docs to game$lastCreatedAt") *>
