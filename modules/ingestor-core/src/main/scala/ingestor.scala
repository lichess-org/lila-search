...
object Ingestor:
  ...
  // For total translation functions: B => A
  def apply[A: Schema, B](
      index: Index,
      repo: Repo[B],
      translate: B => A,
      store: KVStore,
    ...
    def watch: IO[Unit] =
      fs2.Stream
        ...
        .evalMap: result =>
          ...
          updateElastic(translated, false) *> saveLastIndexedTimestamp(result.timestamp)
        .compile
        .drain
    ...
    def watch(since: Option[Instant], dryRun: Boolean): IO[Unit] =
      repo
        .watch(since)
        .evalMap: result =>
          updateElastic(result.map(translate), dryRun)
        .compile
        .drain
  ...
  // For partial translation functions: B => Option[A]
  def applyPartial[A: Schema, B](
      index: Index,
      repo: Repo[B],
    ...
    def watch(since: Option[Instant], dryRun: Boolean): IO[Unit] =
      repo
        .watch(since)
        .evalMap: result =>
          updateElastic(result.flatMap(translate), dryRun)
        .compile
        .drain
