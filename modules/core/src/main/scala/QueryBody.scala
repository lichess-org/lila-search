...
object Q {
  implicit val query: Query[Q] =
    new Query[Q] {
      def searchDef(query: Q)(from: From, size: Size) =
        query match {
...
case class Team(text: String) extends Q
...
case class Forum(text: String, troll: Boolean) extends Q

case class Study(text: String, userId: Option[String]) extends Q
